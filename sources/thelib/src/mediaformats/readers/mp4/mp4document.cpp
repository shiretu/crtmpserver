...
#include "mediaformats/readers/mp4/atomctts.h"
...
#include "mediaformats/readers/mp4/atommfhd.h"
#include "mediaformats/readers/mp4/atomminf.h"
#include "mediaformats/readers/mp4/atommoof.h"
...
#include "mediaformats/readers/mp4/atomtraf.h"
...
#include "mediaformats/readers/mp4/atomvmhd.h"
...
MP4Document::MP4Document(Metadata &metadata)
...
bool MP4Document::BuildFrames() {
	...
	if (pESDS != NULL) {
		if (pESDS->IsMP3()) {
			if (_streamCapabilities.AddTrackAudioMP3(0, 0, 0, NULL) == NULL) {
				FATAL("Unable to initialize MP3 codec");
				return false;
			}
		} else {
			...
			audioHeader.pts = audioHeader.dts = audioHeader.cts = 0;
			...
			if (_streamCapabilities.AddTrackAudioAAC(GETIBPOINTER(raw),
					(uint8_t) GETAVAILABLEBYTESCOUNT(raw), true, NULL) == NULL) {
	...
	if (pAVCC != NULL) {
		...
		videoHeader.pts = videoHeader.dts = videoHeader.cts = 0;
		...
		if (_streamCapabilities.AddTrackVideoH264(
				GETIBPOINTER(raw) + 8,
				spsLength,
...
bool MP4Document::BuildMOOVFrames(bool audio) {
	...
	vector<uint64_t> chunkOffsets;
	if (pSTCO != NULL)
		chunkOffsets = pSTCO->GetEntries();
	else
		chunkOffsets = pCO64->GetEntries();
	vector<uint32_t> sample2Chunk = pSTSC->GetEntries((uint32_t) chunkOffsets.size());
	...
	uint32_t iterrationCount = (uint32_t) sampleSize.size();
	iterrationCount = iterrationCount > sample2Chunk.size()
			? (uint32_t) sample2Chunk.size() : iterrationCount;
	...
	for (uint32_t i = 0; i < iterrationCount; i++) {
		...
		frame.start = chunkOffsets[sample2Chunk[i]] + localOffset;
		if (pCTSS != NULL) {
		...
		//frame.isKeyFrame = true;
		frame.dts = ((double) totalTime / (double) timeScale)*(double) 1000.00;
		frame.pts = frame.dts + frame.cts;
...
Variant MP4Document::GetPublicMeta() {
	Variant result;
