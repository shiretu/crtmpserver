...
#include "mediaformats/readers/mp3/id3parser.h"
...
MP3Document::MP3Document(Metadata &metadata)
	...
	while (_mediaFile.Cursor() != _mediaFile.Size()) {
		...
		if ((firstBytes[0] == 0xff) &&
			...
			if (!_capabilitiesInitialized) {
				if (_streamCapabilities.AddTrackAudioMP3(
						_samplingRates[version][sampleRateIndex], 2, 0, NULL) == NULL) {
					FATAL("Unable to initialize stream capabilities for MP3");
					return false;
				}
			...
			if (layer == LAYER_1)
				...
				samplesCount = 1152;
			frame.pts = frame.dts = frame.cts = totalDuration * 1000;
...
Variant MP3Document::GetPublicMeta() {
	return _tags;
}
